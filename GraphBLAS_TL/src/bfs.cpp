...
int main() {
    LoadedMatrix edgeLists = LoadUnweightedMatrix("ValidationGraphs/bfs/dir-input");
    auto matrix = FillMatrix(edgeLists);
...
void bfs(
        VectorT &result,
        MatrixT& matrix,
        GraphBLAS::IndexType startVertex,
        bool isWcc,
        int wccLevel) {

    VectorT visited{matrix.ncols(), 0};
    VectorT neighbours{matrix.ncols(), 0};
}
...
//        }
//
//        // neighbors<!visited> = A ||.&& neighbors ; finds all the unvisitedw
